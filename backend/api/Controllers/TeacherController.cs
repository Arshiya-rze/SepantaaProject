...
public class TeacherController(ITeacherRepository _teacherRepository, ITokenService _tokenService) : BaseApiController
{
    ...
    public async Task<ActionResult<Attendence>> Add(string targetStudentUserName, AddStudentStatusDto studentInput, CancellationToken cancellationToken)
    {
        ...
        if (studentInput is null) return BadRequest("No Times selected");

        Attendence? attendence = await _teacherRepository.AddAsync(targetStudentUserName, studentInput, cancellationToken);
